...
N-word ``CaptionGroup``, this module emits N events, each spanning exactly
one word's own ``[start_s, end_s)`` and rendering all N words of the group,
with only that event's word wrapped in an accent-colour override
...
# primary colour, accent colour, outline, and shadow. Those six are read from
# ``style`` with the defaults below when absent. Every other field the ASS
# ``[V4+ Styles]`` line requires (SecondaryColour, OutlineColour, BackColour,
# Bold, Italic, ..., Alignment, the three margins, Encoding) is not part of
# that enumerated six, so it is a fixed constant here rather than a second,
# unnamed style contract this module would be inventing on Tasks 11/12's
# behalf. See this task's report for the full list and the reasoning.
...
_ALIGNMENT = "2"  # bottom-centre, numpad-style
_MARGIN_L = "40"
_MARGIN_R = "40"
_MARGIN_V = "80"
_ENCODING = "1"
...
def _style_field(style: Mapping[str, object], key: str, default: object) -> str:
    """One of the six style-driven ``[V4+ Styles]``/override fields:
...
def _style_line(style: Mapping[str, object]) -> str:
    ...
    ``style``'s five style-line fields (font name, size, primary colour,
    outline, shadow - ``accent_colour`` is not a ``[V4+ Styles]`` field; it
    only ever appears in a per-word override, see ``_render_words``) plus
    the fixed fields listed at module scope.
    """
    fields = ",".join(
        [
            _STYLE_NAME,
...
def render_ass(timeline: Timeline, *, width: int, height: int, style: Mapping[str, object]) -> str:
    ...
    supplies the six caller-configurable fields (see the module-level
